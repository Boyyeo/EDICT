...
## Becareful to the scaling factor
...
def baseline_stablediffusion(prompt="",
    ...
    if init_image is not None:
        ...
        with autocast(device):
            init_latent = vae.encode(init_image).latent_dist.sample(generator=generator) * 1.0000 
    ...
    if prev_image is not None:
        ...
        #Encode image
        with autocast(device):
            prev_init_latent = vae.encode(prev_image).latent_dist.sample(generator=generator) * 1.0000 
    ...
    #Process clip
    with autocast(device):
        tokens_unconditional = tokenizer(null_prompt, padding="max_length", max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt", return_overflowing_tokens=True)
        embedding_unconditional = text_encoder(tokens_unconditional.input_ids.to(device)).last_hidden_state

        tokens_conditional = tokenizer(prompt, padding="max_length", max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt", return_overflowing_tokens=True)
        embedding_conditional = text_encoder(tokens_conditional.input_ids.to(device)).last_hidden_state

        #Process prompt editing
        assert not ((prompt_edit is not None) and (prev_image is not None))
        if prompt_edit is not None:
            tokens_conditional_edit = tokenizer(prompt_edit, padding="max_length", max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt", return_overflowing_tokens=True)
            embedding_conditional_edit = text_encoder(tokens_conditional_edit.input_ids.to(device)).last_hidden_state
...
def latent_to_image(latent):
    image = vae.decode(latent.to(vae.dtype)/1.0000 ).sample
...
def coupled_stablediffusion(prompt="",
    ...
    def image_to_latent(im, init_noise_sigma=1.0):
        if isinstance(im, torch.Tensor):
            ...
            im = im.to(device)
            #Encode image
            im = im.to(dtype=vae.dtype)

            init_latent = vae.encode(im).latents * init_noise_sigma
            return init_latent
    ...
    tokens_unconditional = tokenizer(null_prompt, padding="max_length",
    ...
    uncond_prompt_embeds, embedding_unconditional = text_encoder(input_ids=tokens_unconditional.input_ids.to(device),attention_mask=tokens_unconditional.attention_mask.to(device))
    #print("embedding_unconditional:{} null_prompt:{} negative_prompt_embeds:{}".format(embedding_unconditional.shape,null_prompt,uncond_prompt_embeds.shape))
    
    tokens_conditional = tokenizer(prompt, padding="max_length", 
                                        max_length=tokenizer.model_max_length,
                                        truncation=True, return_tensors="pt", 
                                        return_overflowing_tokens=True)
   
   # print("tokens_conditional_input_ids:",tokens_conditional.shape)
    #embedding_conditional = text_encoder(tokens_conditional.input_ids.to(device)).last_hidden_state

    cond_prompt_embeds, embedding_conditional = text_encoder(input_ids=tokens_conditional.input_ids.to(device),attention_mask=tokens_conditional.attention_mask.to(device))


    cond_image_embeds, uncond_image_embeds = pipe_prior(prompt, null_prompt, guidance_scale=1.0, generator=torch.Generator(device=device).manual_seed(seed)).to_tuple()

    #Process prompt editing (if running Prompt-to-Prompt)
    if prompt_edit is not None:
        tokens_conditional_edit = tokenizer(prompt_edit, padding="max_length", 
                                                 max_length=tokenizer.model_max_length,
                                                 truncation=True, return_tensors="pt", 
                                                 return_overflowing_tokens=True)
        embedding_conditional_edit = text_encoder(tokens_conditional_edit.input_ids.to(device)).last_hidden_state
    ...
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
        ...
        for latent_i in range(2): 
            ...
            added_cond_kwargs = {"text_embeds": uncond_prompt_embeds, "image_embeds": uncond_image_embeds}
            #print("text_embeds:{} image_embeds:{} latent:{} emb:{}".format(uncond_prompt_embeds.shape,uncond_image_embeds.shape,latent_model_input.shape,embedding_unconditional.shape))
            #Predict the unconditional noise residual
            noise_pred_uncond = unet(latent_model_input, t, 
                                     encoder_hidden_states=embedding_unconditional,added_cond_kwargs=added_cond_kwargs).sample
            ...
            added_cond_kwargs = {"text_embeds": cond_prompt_embeds, "image_embeds": cond_image_embeds}
            #Predict the conditional noise residual and save the cross-attention layer activations
            noise_pred_cond = unet(latent_model_input, t, 
                                   encoder_hidden_states=embedding_conditional,added_cond_kwargs=added_cond_kwargs).sample
            
            noise_pred_cond, _  = noise_pred_cond.split(latent_model_input.shape[1], dim=1)
            ...
            #print("noise_pred:{} latent_base:{}".format(noise_pred.shape,latent_base.shape))
    ...
    images = []
    for latent_i in range(2):
        latent = latent_pair[latent_i] / 1.0000
        image = vae.decode(latent.to(vae.dtype)).sample
        images.append(image)
